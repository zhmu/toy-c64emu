...
unsigned int
Disk::CalculateOffset(unsigned int track, unsigned int sector)
{
...
Disk::Load(const char* fname)
{
	...
	TRACE("Disk::Load(): loaded '%s', %u tracks\n", fname, m_num_tracks);
...
bool
Disk::Entry::Matches(const char* name) const
{
...
uint8_t*
Disk::GetDataPointer(unsigned int track, unsigned int sector)
{
	return (uint8_t*)(m_data + CalculateOffset(track, sector));
}

const uint8_t*
Disk::GetDataPointer(unsigned int track, unsigned int sector) const
{
...
bool
Disk::GetDirectoryEntry(int n, Entry& entry) const
{
...
bool
Disk::FindDirectoryEntry(const char* name, Entry& entry) const
{
	for (int n = 0; /* nothing */; n++) {
		if (!GetDirectoryEntry(n, entry))
			return false;
...
bool
Disk::OpenFile(const char* name, Handle& handle)
{
...
bool
Disk::Handle::IsValid() const
{
	return m_disk != NULL;
}

bool
Disk::Handle::AtEOF() const
{
	return m_file_offset >= m_entry.m_length * Disk::s_sector_size;
}

uint8_t
Disk::Handle::GetNextByte()
{
	...
	if (m_file_offset % Disk::s_sector_size == 0) {
		...
		TRACE("Disk::Handle::GetNextByte(): cur track/sector = %u/%u\n", m_cur_track, m_cur_sector);
	}
	...
	if (m_file_offset % Disk::s_sector_size == 0) {
		...
		m_next_track = *data++;
		m_next_sector = *data++;
		TRACE("Disk::Handle::GetNextByte(): next track/sector = %u/%u\n", m_next_track, m_next_sector);
		m_file_offset += 2;
	}
	...
	TRACE("Disk::Handle::GetNextByte(): byte = 0x%x\n", *data);
	m_file_offset++;
	return *data;
}

void
Disk::Entry::Dump() const
{
