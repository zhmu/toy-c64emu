...
void
IECBus::OnChangeDataPortA(uint8_t value)
{
	TRACE("IECBus::HandleDataPortA(): state=%u, val=0x%x => %c%c%c\n",
	 m_state, value,
	 (value & s_PortA_DataOut) ? 'D' : '.',
	 (value & s_PortA_ClockOut) ? 'C' : '.',
	 (value & s_PortA_ATNOut) ? 'A' : '.');
	...
	if ((prev_bus_c64 & s_Bus_ATN) != 0 && (m_bus_c64 & s_Bus_ATN) == 0) {
		if (m_talk_pending) {
			...
			TRACE("IECBus::OnChangeDataPortA(): device became TALKER -> ST_TURNAROUND\n");
		}
	}
...
IECBus::OnReadDataPortA()
{
	...
	TRACE("IECBus::OnReadDataPortA(): state=%u, c64bus=%c%c%c, devbus=%c%c, val=0x%x\n",
	 m_state,
	 (m_bus_c64 & s_Bus_Data) ? 'D' : '.',
	 (m_bus_c64 & s_Bus_Clock) ? 'C' : '.',
	 (m_bus_c64 & s_Bus_ATN) ? 'A' : '.',
	 (m_bus_device & s_Bus_Data) == 0 ? 'D' : '.',
	 (m_bus_device & s_Bus_Clock) == 0 ? 'C' : '.',
	 result);
	return result;
}
...
IECBus::Update()
{
	...
	if (m_state == ST_ATN && IS_CLOCK_HI) {
		TRACE("IECBus::UpdateListener: ATN signalled, CLK signalled -> resetting\n");
		Reset();
		return;
	}
	...
	switch(m_state) {
		case ST_IDLE:
			...
			if (m_bus_timeout > 0) {
				// Need to timeout
				if (--m_bus_timeout == 0) {
					TRACE("IECBus::UpdateListener: ST_IDLE expired -> ST_DATA_HI\n");
					m_state = ST_DATA_HI;
					SET_DATA_ON;
				}
			}
		...
		case ST_DATA_HI:
			...
			if (IS_CLOCK_LO) {
				...
				SET_DATA_OFF;
				TRACE("IECBus::UpdateListener: acknowledged /CLK; ready for CLK -> WAIT_EOI\n");
				m_state = ST_WAIT_EOI;
				m_bus_timeout = 100;
			}
			break;
		case ST_WAIT_EOI:
			...
			if (IS_CLOCK_HI) {
				...
				TRACE("IECBus::UpdateListener: CLK while waiting for EOI, no eoi -> ST_TRANSFER\n");
				m_state = ST_TRANSFER;
				m_eoi = false;
				...
				TRACE("IECBus::UpdateListener: no CLK, timeout expired: eoi signalled -> ST_ACK_EOI\n");
				m_state = ST_ACK_EOI;
				m_eoi = true;
				m_bus_timeout = 10;
			}
		...
		case ST_ACK_EOI:
			...
			if (IS_CLOCK_HI) {
				...
				TRACE("IECBus::UpdateListener: got CLK -> ST_TRANSFER\n");
				m_state = ST_TRANSFER;
			}
			if (--m_bus_timeout == 0) {
				SET_DATA_OFF;
				TRACE("IECBus::UpdateListener: EOI timeout expired -> ST_TRANSFER\n");
				m_state = ST_TRANSFER;
			}
			break;
		case ST_ACK:
			if (--m_bus_timeout == 0) {
				...
				if (m_eoi) {
					TRACE("IECBus::UpdateListener: done ACK, eoi active -> ST_IDLE\n");
					SET_DATA_OFF;
					m_state = ST_IDLE;
				} else {
					TRACE("IECBus::UpdateListener: done ACK, no eoi -> ST_DATA_HI\n");
					m_state = ST_DATA_HI;
				}
			}
			break;
...
void
IECBus::ProcessCommandByte(uint8_t byte)
{
	uint8_t channel = byte & 0xf;
	switch((byte & 0xf0)) {
		case s_Command_Open:
			TRACE("IECBus::ProcessCommandByte(): got OPEN (%x)\n", byte);
			if (m_curdevice != NULL)
				m_curdevice->OnOpen(channel);
			return;
		case s_Command_Close:
			TRACE("IECBus::ProcessCommandByte(): got CLOSE (%x)\n", byte);
			if (m_curdevice != NULL)
				m_curdevice->OnClose(channel);
			return;
		case s_Command_Listen:
			TRACE("IECBus::ProcessCommandByte(): got LISTEN (%x), channel %x\n", byte, byte & 0xf);
			m_devstate = DS_LISTEN;
			m_curdevice = FindDevice(byte & 0xf);
			return;
		case s_Command_Talk:
			TRACE("IECBus::ProcessCommandByte(): got TALK (%x), channel %x\n", byte, byte & 0xf);
			m_talk_pending = true;
			m_curdevice = FindDevice(byte & 0xf);
			return;
		case s_Command_OpenChannel:
			TRACE("IECBus::ProcessCommandByte(): got OPEN CHANNEL (%x), channel %x\n", byte, byte & 0xf);
			if (m_curdevice != NULL)
	...
	switch(byte) {
		case s_Command_Unlisten:
			TRACE("IECBus::ProcessCommandByte(): got UNLISTEN (%x)\n", byte);
			m_curdevice = NULL;
			return;
		case s_Command_Untalk:
			TRACE("IECBus::ProcessCommandByte(): got UNTALK (%x)\n", byte);
			m_curdevice = NULL;
			return;
		default:
			TRACE("IECBus::ProcessCommandByte(): unrecognized byte 0x%x, ignored\n", byte);
			break;
	}
}
...
IECBus::UpdateTalker()
{
	switch(m_state) {
			...
			if (IS_DATA_HI) {
				...
				if (m_bus_timeout > 0 && --m_bus_timeout == 0) {
					if (!m_eoi) {
						m_state = ST_DATA_HI;
						SET_CLK_OFF;
						TRACE("IECBus::UpdateTalker(): data hi -> ST_DATA_HI\n");
					} else {
						/*
						 ...
						 * /ATN is signalled.
						 */
						TRACE("IECBus::UpdateTalker(): no data to send -> signalling error\n");
						SET_CLK_OFF;
						SET_DATA_OFF;
					}
				}
		...
		case ST_DATA_HI:
			...
			if (IS_DATA_LO) {
				...
				if (!m_eoi) {
					...
					TRACE("IECBus::UpdateTalker(): data lo, device ready, no eoi -> ST_TRANSFER_WAIT\n");
					...
					TRACE("IECBus::UpdateTalker(): data lo, device ready, eoi -> ST_WAIT_EOI\n");
					m_bus_timeout = 100;
					m_state = ST_WAIT_EOI;
				}
			}
		...
		case ST_WAIT_EOI:
			...
			if (IS_DATA_HI) {
				TRACE("IECBus::UpdateTalker(): got EOI acknowledge -> ST_ACK_EOI\n");
				m_state = ST_ACK_EOI;
			}
			break;
		case ST_ACK_EOI:
			...
			if (IS_DATA_LO) {
				TRACE("IECBus::UpdateTalker(): data lo -> ST_TRANSFER_WAIT\n");
				SET_CLK_ON;
				m_state = ST_TRANSFER_WAIT;
				m_bus_cur_bit = 0;
				m_bus_timeout = 50;
		...
		case ST_ACK:
			...
			if (IS_DATA_HI) {
				...
				TRACE("IECBus::UpdateTalker(): data hi -> ST_IDLE\n");
				m_state = ST_IDLE;
				m_bus_timeout = 10;
		...
		case ST_TRANSFER_WAIT:
			...
			if (--m_bus_timeout == 0) {
				TRACE("IECBus::UpdateTalker(): timeout expired -> ST_TRANSFER\n");
				m_state = ST_TRANSFER;
			}
			break;
		case ST_TRANSFER_DATA:
			...
			if (--m_bus_timeout == 0) {
				...
				if (m_bus_cur_bit == 8) {
					TRACE("IECBus::UpdateTalker(): all bits sent -> ST_ACK\n");
					m_state = ST_ACK;
				} else {
					TRACE("IECBus::UpdateTalker(): timeout expired -> ST_TRANSFER_WAIT\n");
					m_state = ST_TRANSFER_WAIT;
				}
			}
			break;
		case ST_TRANSFER:
			TRACE("IECBus::UpdateTalker(): timeout expired, transferring bit %u\n", m_bus_cur_bit);
		...
		case ST_TURNAROUND:
			...
			if (IS_DATA_HI && IS_CLOCK_LO) {
				TRACE("IECBus::UpdateListener: ATN turnaround, DATA hi, CLK inactive\n");
				SET_DATA_OFF; SET_CLK_ON;
				m_state = ST_IDLE;
				m_bus_timeout = 10;
...
void
IECBus::ProcessDataByte(uint8_t byte)
{
	TRACE("IECBus::ProcessDataByte(): got byte 0x%x (%c)\n", byte, byte);
	if (m_curdevice != NULL)
		m_curdevice->OnDataByte(byte);
}

IECDevice*
IECBus::FindDevice(uint8_t id)
{
...
void
IECBus::RegisterDevice(int id, IECDevice* pDevice)
{
